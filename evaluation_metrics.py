...
parser.add_argument('--gen_dir', default='./checkpoints/0530110758_res9_cont_decay/images/test/', type=str)
...
class GANDataset(Dataset):
    ...
    def __init__(self, rootA, rootB, transform=None, unaligned=False, device=None):
        ...
        sortedA = sorted(os.listdir(rootA), key=lambda name: int(name.split('.')[0].split('_')[-1]))
...
def SSIM(img_gen, img_gt):
    """
      implementation borrowed from https://ece.uwaterloo.ca/~z70wang/research/ssim/
    """
    ssim_score = pytorch_ssim.ssim(img_gen, img_gt)
...
if __name__ == '__main__':
    ...
    for i, (img_A, img_B, index_A, index_B) in enumerate(dataloader):
        ...
        SSIM_score = SSIM(img_A, img_B)

        PSNR_scores.append(PSNR_score)
        SSIM_scores.append(SSIM_score)

    print('The average PSNR score of the generated images are: ', torch.stack(PSNR_scores).mean())
    print('The average SSIM score of the generated images are: ', torch.stack(SSIM_scores).mean())