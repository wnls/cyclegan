...
class CycleGANModel:
    ...
    def __init__(self, args):
        ...
        self.wgan_c = args.wgan_c
        self.d_steps = args.d_steps
        self.g_steps = args.g_steps

        if self.use_wgan:
            self.optimizer_G = torch.optim.RMSprop(itertools.chain(self.G_A.parameters(),
                                                   self.G_B.parameters()),
                                                   lr=args.lr, alpha=0.99)
            self.optimizer_D = torch.optim.RMSprop(itertools.chain(self.D_A.parameters(),
                                                   self.D_B.parameters()),
                                                   lr=args.lr, alpha=0.99)
        else:
    ...
    def train(self, input, save, out_dir_img, epoch):
        ...
        for g_idx in range(self.g_steps):
            ...
            B_gen = self.G_A(A)
            if self.use_wgan:
                loss_G_A = - torch.mean(self.D_A(B_gen))
            else:
            ...
            A_gen = self.G_B(B)
            if self.use_wgan:
                loss_G_B = - torch.mean(self.D_B(A_gen))
            else:
        ...
        for d_idx in range(self.d_steps):
            ...
            # D_A real loss
            if self.use_wgan:
                loss_D_A_real = -torch.mean(self.D_A(B))
            else:
            ...
            if self.use_wgan:
                loss_D_A_fake = torch.mean(self.D_A(B_gen_pool))
            else:
            ...
            # D_B real loss
            if self.use_wgan:
                loss_D_B_real = -torch.mean(self.D_B(A))
            else:
            ...
            for p in self.D_A.parameters():
                p.data.clamp_(-self.wgan_c, self.wgan_c)
